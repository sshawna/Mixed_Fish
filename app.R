               ...
               tabPanel(" Landings",value = "mi", icon = icon("fish"), 
                        tabsetPanel(id="Ltabselected", type="pills",
                                    tabPanel("Page1", value="page1",  
                                             fluidRow(
                                               column(width=12,offset=7,div(style="display: inline-block;vertical-align:top; width: 200px;",selectInput("set1", label = "Select Metier",c("None",levels(test$Metier_lvl5)),selectize = T))
                                               )),
                                             fluidRow(column(width = 7,ggiraph::ggiraphOutput("plotL1")
                                             ),
                                             column(width = 5, plotlyOutput("plotL1param",height = "400px")))),
                                    tabPanel("Page2", value="page2"  ,  
                                             fluidRow(
                                               column(width=12,offset=7,div(style="display: inline-block;vertical-align:top; width: 200px;",selectInput("set2", label = "Select Species",c("None",levels(test$Species)),selectize = T))
                                                      
                                               )
                                             ),
                                             fluidRow(column(width = 7,ggiraph::ggiraphOutput("plotL2")
                                             ),column(width = 5, plotlyOutput("plotL2param",height = "400px"))
                                             )))),
               
               tabPanel(" Effort", value = "sb", icon = icon("ship"),
                        fluidRow(
                          column(width=12,offset=7,div(style="display: inline-block;vertical-align:top; width: 200px;",selectInput("set3", label = "Select Metier",c("None",levels(testE$Metier_lvl5)),selectize = T))
                          )),
                        fluidRow(column(width = 7,ggiraph::ggiraphOutput("plotE1")
                        ),
...
server <- function(input, output, session) {
  observeEvent(input$about, {
    shinyalert(
      ...
      text = "Vizualization Tool for Mixed Fisheries Landings and Effort in Celtic Seas Ecoregion. <br> Try changing the filters on the panel to compare different <b>Metier</b> and <b>Species</b> by <b>years</b> .",
      closeOnEsc = TRUE,
  ...
  for(i in 1:length(a)){
    ...
    compplot <- ggplot(test1, aes(Year,OfficialLanW, fill=Species)) + 
      ...
      ylab("Total Landings") +
      xlab("Year") + ggtitle(a[i])+ scale_x_continuous(breaks=test1$Year)+
      
      #geom_bar(stat = "identity") +
      #facet_wrap(~ FishActEUivl5) +
      theme_grey(base_size = 16) + 
      viridis::scale_fill_viridis(discrete = TRUE) +theme(legend.text=element_text(size=10),
                                                          strip.background = element_blank(),axis.text.x = element_text(angle = 90, hjust = 1))
  ...
  for(i in 1:length(a1)){
    ...
    compplot <- ggplot(test1, aes(Year,OfficialLanW, fill=Metier_lvl5)) + 
      ...
      #geom_bar_interactive(stat = "identity",aes(tooltip = Species))+
      ylab("Total Landings") +
      xlab("Year") + ggtitle(a1[i])+ scale_x_continuous(breaks=test1$Year)+
  ...
  output$plotE1 <-renderggiraph({
    ...
    compplot <- ggplot(testE, aes(Year,KWperDay, fill=VesselLen)) + 
      ...
      facet_wrap(~ Metier_lvl5) +
      theme_grey(base_size = 16) + 
      viridis::scale_fill_viridis(discrete = TRUE) +theme(legend.position="bottom", 
                                                          legend.text=element_text(size=10),
                                                          strip.background = element_blank(),axis.text.x = element_text(angle = 90, hjust = 1))
    x <- girafe(code = print(compplot), width_svg = 13, height_svg = 9)
    x <- girafe_options(x, opts_selection(
